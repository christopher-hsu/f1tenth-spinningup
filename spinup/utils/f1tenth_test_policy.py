...
def run_policy(env, get_action, env_init, ego_agent, opp_agent, 
    ...
    while n < num_episodes:
        ...
        if d or (ep_len == max_ep_len):
            ...
            init_positions = np.random.random_integers(0,1)
            o, r, d, ep_ret, ep_len = env.reset({'x': env_init['initial_x'][init_positions],
                                                 'y': env_init['initial_y'],
...
def run_adversarial_policy(env, ego_action, opp_action, env_init, ego_agent, opp_agent, 
                    max_ep_len=None, num_episodes=100, render=True):
    ...
    while n < num_episodes:
        ...
        # Take deterministic actions at test time 
        a = ego_action(RLobs, action_mask=ego_agent.aval_paths, deterministic=True)
        ego_speed, ego_steer, a = ego_agent.plan(o, a)
